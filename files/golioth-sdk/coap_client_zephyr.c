...
static const char *cid_status_str(int status)
{
    switch (status)
    {
...
static void log_dtls_cid_status(int sock, const char *stage)
{
#if defined(TLS_DTLS_CID_STATUS)
    int status = -1;
    socklen_t optlen = sizeof(status);
    int ret = zsock_getsockopt(sock, SOL_TLS, TLS_DTLS_CID_STATUS, &status, &optlen);
    if (ret < 0)
    {
        GLTH_LOGW(TAG, "DTLS CID status read failed at %s: %d", stage, -errno);
        return;
    }

    GLTH_LOGI(TAG, "DTLS CID status at %s: %s (%d)", stage, cid_status_str(status), status);
#else
    ARG_UNUSED(sock);
    ARG_UNUSED(stage);
#endif
...
static void log_dtls_peer_cid(int sock, const char *stage)
{
#if defined(TLS_DTLS_PEER_CID_VALUE)
    uint8_t peer_cid[32];
    socklen_t optlen = sizeof(peer_cid);
    int ret = zsock_getsockopt(sock, SOL_TLS, TLS_DTLS_PEER_CID_VALUE, peer_cid, &optlen);
    if (ret < 0)
    {
        GLTH_LOGW(TAG, "DTLS peer CID read failed at %s: %d", stage, -errno);
        return;
    }

    GLTH_LOGI(TAG, "DTLS peer CID length at %s: %u", stage, (unsigned int) optlen);
#else
    ARG_UNUSED(sock);
    ARG_UNUSED(stage);
#endif
...
static int golioth_send(struct golioth_client *client, uint8_t *data, size_t len, int flags)
{
    ...
    if (sent < 0)
    {
        GLTH_LOGW(TAG, "zsock_send failed sock=%d errno=%d", client->sock, errno);
        return -errno;
    }
    else if (sent < len)
    {
...
static int golioth_setsockopt_dtls(struct golioth_client *client, int sock, const char *host)
{
    ...
    {
        int enabled = TLS_DTLS_CID_ENABLED;
        ...
        GLTH_LOGI(TAG, "DTLS CID enabled on socket");
    }
                                    ...
                                    socklen_t addrlen)
{
    ...
    if (IS_ENABLED(CONFIG_GOLIOTH_USE_CONNECTION_ID))
    {
        log_dtls_cid_status(sock, "post-connect");
        log_dtls_peer_cid(sock, "post-connect");
    }
                                     ...
                                     const char *port)
{
    ...
    if (client->cached_peer_addr_valid)
    {
        err = golioth_connect_sockaddr(client,
                                       host,
                                       (struct sockaddr *) &client->cached_peer_addr,
                                       client->cached_peer_addr_len);
        if (!err)
        {
            LOG_SOCKADDR("Reused cached addr '%s'",
                         (struct sockaddr *) &client->cached_peer_addr);
            return 0;
        }

        client->cached_peer_addr_valid = false;
    }
    ...
    for (addr = addrs; addr != NULL; addr = addr->ai_next)
    {
        ...
        if (!err)
        {
            memcpy(&client->cached_peer_addr, addr->ai_addr, addr->ai_addrlen);
            client->cached_peer_addr_len = addr->ai_addrlen;
            client->cached_peer_addr_valid = true;
            /* Ready to go */
            break;
        }
    }
...
static void golioth_coap_client_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        // Enqueue an asynchronous EMPTY request immediately unless the
        // application explicitly opts out to minimize per-connect overhead.
        if (!IS_ENABLED(CONFIG_GOLIOTH_COAP_CLIENT_SKIP_INITIAL_EMPTY) &&
